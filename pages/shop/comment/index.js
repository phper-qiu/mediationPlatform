...
Page({
  ...
  onLoad: function (options) {
    ...
    app.getUserInfo(function(userInfo){
      ...
      that.setData({
        userInfo: userInfo,
        shop_id: decodeURIComponent(options.shopid)
      });
    });
  ...
  getCommentsList(){
    var that = this
    //all获取全部，good好评medium中评bad差评
    util.request(api.getShopComments, {shop_id:that.data.shop_id,rating:that.data.currentTab,index:10},'POST').then(function (res) {
      if (res.code == 200) {
        that.setData({
